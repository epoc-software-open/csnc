 ...
 * @access      public
 */
class Hospitalinfo_Validator_UpdateCheck extends Validator
{
	/**
	 * validate実行
	 *
	...
	function validate($attributes, $errStr, $params)
	{
		...
		// 施設名の重複チェック
		$hospitalinfo = $hospitalinfoView->getHospitalinfoFromName( trim( $hospital_name, '　' ), $hospitalinfo_id );
		if ( !empty( $hospitalinfo ) ) {
			return "エラー！\n指定された施設名は存在します。\n\nコード[" . $hospitalinfo[0]["hospital_code"] . "]";
		}

		// かなの重複チェック
		$hospitalinfo = $hospitalinfoView->getHospitalinfoFromKana( trim( $hospital_kana, '　' ), $hospitalinfo_id );
		if ( !empty( $hospitalinfo ) ) {
			return "エラー！\n指定された施設名（かな）は存在します。\n\nコード[" . $hospitalinfo[0]["hospital_code"] . "]";
		}

		// コードの重複チェック
		$hospitalinfo = $hospitalinfoView->getHospitalinfoFromCode( trim( $hospital_code, '　' ), $hospitalinfo_id );
		if ( !empty( $hospitalinfo ) ) {
