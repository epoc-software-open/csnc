 ...
 * @access      public
 */
class Hospitalinfo_View_Admin_Search extends Action
{
	// リクエストを受け取るため
	var $limit = null;
	var $offset = null;
    ...
    function execute()
    {
		...
		if($this->count > 0) {
			$sql = "SELECT * FROM {hospitalinfo} " . $sqlwhere . " ORDER BY insert_time DESC";
	...
	function _fetchcallback($result) {
		...
		while ($row = $result->fetchRow()) {
			...
			//$ret[$count]['title'] = "";
			//$ret[$count]['url'] = "#";
			$ret[$count]['action'] = "hospitalinfo_view_main_init";
