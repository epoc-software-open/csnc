 ...
 */
class Testee_Validator_MobileBlock extends Validator
{
	/**
	 * ブロックに配置しているかどうかをチェック
	 *
	...
	function validate($attributes, $errStr, $params)
	{
		...
		$module_name = $modulesView->loadModuleName("testee");

		if (empty($attributes)) {
			return sprintf($errStr, $module_name);
		}
		$mdbView =& $container->getComponent("mdbView");
		$testee_list = $mdbView->getBlocksForMobile($attributes);
		if (empty($testee_list)) {
			return sprintf($errStr, $module_name);
		}

		$actionChain =& $container->getComponent("ActionChain");
		$action =& $actionChain->getCurAction();
		BeanUtils::setAttributes($action, array("testee_list"=>$testee_list));
	}
}
?>