 ...
 * @access      public
 */
class Testee_Action_Edit_Metadataseq extends Action
{
    // リクエストパラメータを受け取るため
    ...
    function execute()
    {
    	$testee_id = $this->drag_metadata['testee_id'];
    	// 移動元
		$where_params = array(
			"testee_id" => $testee_id,
		...
		$result = $this->db->seqExecute("testee_metadata", $where_params, $sequence_param);
		if ($result === false) {
			return 'error';
		}
		...
		$result = $this->db->seqExecute("testee_metadata", $where_params, $sequence_param, 1);
		if ($result === false) {
			return 'error';
		}
		// metadata更新
		...
		$result = $this->db->updateExecute("testee_metadata", $params, $where_params);
		if ($result === false) {
			return 'error';
		}
		return 'success';
