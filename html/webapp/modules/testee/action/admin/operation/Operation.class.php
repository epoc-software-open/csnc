 ...
 * @access      public
 */
class Testee_Action_Admin_Operation extends Action
{
	...
	function execute()
	{
		switch ($this->mode) {
    		case "move":
    			//掲示板チェック
				$where_params = array(
					"testee_id"=> intval($this->unique_id),
					"room_id"=> intval($this->room_id)
				);
    			$result = $this->db->selectExecute("testee", $where_params);
				...
				$result = $this->db->updateExecute("testee", $params, $where_params, false);
				if($result === false) {
					return "false";
				}
				$testee_block_params = array(
				...
				$result = $this->db->updateExecute("testee_block", $testee_block_params, $where_params, false);
				if($result === false) {
					return "false";
				}
    			...
    			$content_id_arr = $this->db->selectExecute("testee_content", $where_params, null, null, null, $func);
    			if(is_array($content_id_arr) && count($content_id_arr) > 0) {
    				...
    				$metadata_content_id_arr = $this->db->selectExecute("testee_metadata_content", $where_params_in, null, null, null, $func);
    				if($metadata_content_id_arr === false) {
						return "false";
					}
    			}
    			
				$result = $this->db->updateExecute("testee_content", $params, $where_params, false);
				if($result === false) {
					return "false";
				}
				$result = $this->db->updateExecute("testee_metadata", $params, $where_params, false);
				if($result === false) {
					return "false";
				}
				if(!empty($where_params_in)) {
					$result = $this->db->updateExecute("testee_comment", $params, $where_params_in, false);
					if($result === false) {
						return "false";
					}
					$result = $this->db->updateExecute("testee_metadata_content", $params, $where_params_in, false);
					if($result === false) {
						return "false";
					}
				}
				if(is_array($metadata_content_id_arr) && count($metadata_content_id_arr) > 0) {
					...
					$result = $this->db->updateExecute("testee_file", $params, $where_params, false);
					if($result === false) {
						return "false";
					}
					
					$func = array($this, "_fetchcallbackUploads");
					$upload_id_arr = $this->db->selectExecute("testee_file", $where_params, null, null, null, $func);
	    			...
	    			$testee_metadata_content = $this->db->selectExecute("testee_metadata_content", $where_params);
					if($testee_metadata_content === false) {
						return "false";
					}
	    			$upload_id_arr = $this->commonOperation->getWysiwygUploads("content", $testee_metadata_content);
