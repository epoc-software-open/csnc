 ...
 * @access      public
 */
class Testee_View_Admin_Search extends Action
{
	// リクエストを受け取るため
	var $block_id = null;
	var $limit = null;
	...
	function execute()
	{
		...
		$sqlwhere .= " AND contents.temporary_flag = ".TESTEE_STATUS_RELEASED_VALUE;
		$sqlwhere .= $this->sqlwhere;
		$sqlwhere .= " GROUP BY contents.content_id ";
		$sql = "SELECT COUNT(*)" .
				" FROM {testee} mdb" .
				" INNER JOIN {testee_block} block ON (mdb.testee_id=block.testee_id)".
				" INNER JOIN {testee_content} contents ON (mdb.testee_id=contents.testee_id)".  
				" INNER JOIN {testee_metadata_content} datas ON (contents.content_id=datas.content_id)" .
		...
		if ($this->count > 0) {
			$sql = "SELECT block.block_id, mdb.testee_id, mdb.title_metadata_id, title.content as title, datas.content as content, contents.*, file.file_name" .
				" FROM {testee} mdb" .
				" INNER JOIN {testee_block} block ON (mdb.testee_id=block.testee_id)".
				" INNER JOIN {testee_content} contents ON (mdb.testee_id=contents.testee_id)".  
				" INNER JOIN {testee_metadata_content} datas ON (contents.content_id=datas.content_id)" .
				" LEFT JOIN {testee_metadata_content} title ON (mdb.title_metadata_id=title.metadata_id AND datas.content_id=title.content_id)" .
				" LEFT JOIN {testee_file} file ON (title.metadata_content_id=file.metadata_content_id)" .
			...
			$metadatas = $this->db->selectExecute("testee_metadata", $where_params, null, null, null, array($this, '_fetchMetadataCallback'));
