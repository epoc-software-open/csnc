 ...
 * @access      public
 */
class Testee_View_Main_Search_Result extends Action
{
	// リクエストパラメータを受け取るため
	var $block_id = null;
	var $testee_id = null;
	...
	{
		if ($this->back == _ON) {
			$this->keyword = $this->session->getParameter(array('testee', 'search', $this->block_id, 'keyword'));
			$this->select_kind = $this->session->getParameter(array('testee', 'search', $this->block_id, 'select_kind'));
			$this->date_from = $this->session->getParameter(array('testee', 'search', $this->block_id, 'date_from'));
			$this->date_to = $this->session->getParameter(array('testee', 'search', $this->block_id, 'date_to'));
			$this->handle = $this->session->getParameter(array('testee', 'search', $this->block_id, 'handle'));
			$this->status = $this->session->getParameter(array('testee', 'search', $this->block_id, 'status'));
			$this->sort_section = $this->session->getParameter(array('testee', 'search', $this->block_id, 'sort_section'));
			$this->sort_metadata = $this->session->getParameter(array('testee', 'search', $this->block_id, 'sort_metadata'));
			$this->now_page = $this->session->getParameter(array('testee', 'search', $this->block_id, 'now_page'));
		} else {
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'keyword'), $this->keyword);
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'select_kind'), $this->select_kind);
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'date_from'), $this->date_from);
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'date_to'), $this->date_to);
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'handle'), $this->handle);
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'status'), $this->status);
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'sort_section'), $this->sort_section);
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'sort_metadata'), $this->sort_metadata);
			$this->session->setParameter(array('testee', 'search', $this->block_id, 'now_page'), $this->now_page);
		}
		...
		if(!empty($this->date_from)) {
			...
			$sqlwhere .= " AND {testee_content}.insert_time >= ".$fm_insert_date." ";
		...
		if(!empty($this->date_to)) {
			...
			$sqlwhere .= " AND {testee_content}.insert_time <= ".$to_insert_date." ";
		...
		if($this->handle != "") {
			...
			if(is_array($handles)) {
				foreach (array_keys($handles) as $i) {
					$handlesql[] = " {testee_content}.insert_user_name LIKE '%".$handles[$i]."%' ";
				}
			}
		...
		if($this->status != "") {
			if($this->status != TESTEE_STATUS_ALL) {
				$sqlwhere .= " AND {testee_content}.agree_flag=".$this->status." ";
			}
		}
		...
		$this->result_count = $this->mdbView->getMDBListCount($this->testee_id,
																$metadatas,
																$where_params,
																$sqlwhere,
																$keywordBindValues);
		$this->setPageInfo($this->result_count, TESTEE_SEARCH_RESULT_VISIABLE_NUMBER, $this->now_page);
		$this->result_contents = $this->mdbView->getSearchResult($this->testee_id,
																	$metadatas,
																	$where_params,
																	$order_params,
																	$sqlwhere,
																	TESTEE_SEARCH_RESULT_VISIABLE_NUMBER,
																	$this->disp_begin,
																	$keywordBindValues);
