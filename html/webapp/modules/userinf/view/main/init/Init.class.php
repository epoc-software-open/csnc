 ...
 * @access      public
 */
class Userinf_View_Main_Init extends Action
{
    // リクエストパラメータを受け取るため
    var $user_id = null;
    ...
    function execute()
    {
    	...
    	//if($this->user_auth_id == null) $this->user_auth_id = $this->user['user_authority_id'];
    	...
    	// ハンドル名チェック(update_user_name-insert_user_nameが正しい保障がないため、会員管理ではチェックする)
		...
		//プライベートメッセージの利用権限チェック
		$this->pm_use_flag = _ON;
		if ($this->user['user_authority_id'] < _AUTH_GENERAL
			|| $this->session->getParameter('_user_auth_id') < _AUTH_GENERAL) {
			$this->pm_use_flag = _OFF;
			return 'success';
		}
		$pm_module = $this->modulesView->getModuleByDirname('pm');
		if(!isset($pm_module['module_id'])) {
			$this->pm_use_flag = _OFF;
			return 'success';
		}
		$where_params = array('{modules}.module_id' => $pm_module['module_id']);
		$authorities = $this->authoritiesView->getAuthoritiesModulesLinkByAuthorityId($this->user['role_authority_id'], $where_params);
