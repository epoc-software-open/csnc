  ...
  toQueryParams: function(separator) {
    ...
    return match[1].split(separator || '&').inject({}, function(hash, pair) {
      if ((pair = pair.split('='))[0]) {
        ...
        //undefinedを返さない By Change NetCommons:Ryuji.M
...
Element.Methods = {
  ...
  remove: function(element) {
    element = $(element);
    //IEにおいてformタグがあるとメモリリークになるため、修正 by Ryuji
    if(element) {
  ...
  setStyle: function(element, style) {
    ...
    for (var name in style) {
      ...
      if(browser.isNS && name.camelize() == 'opacity') {
      	...
      	//element.style.KhtmlOpacity = value; // TODO some net sources have KHTMLOpacity
      }
    }
    return element;
  },
  ...
  //親エレメント取得
  // object 現オブジェクト
  // nodeNum 何階層か上の親エレメント取得 default=1
  ...
  //子エレメント取得
  // object 現オブジェクト
  // nodeNum 何階層か下の子エレメント取得 default=1
  ...
  //追加
  //element以下のオブジェクトから指定されたclassNameのelementを返す
  ...
  //追加
  //elementより親のオブジェクトから指定されたclassNameのelementを返す
  ...
  serializeElements: function(elements, getHash) {
    var data = elements.inject({}, function(result, element) {
      if (!element.disabled && element.name) {
        var key = element.name, value = Form.Element.getValue($(element));	//iframeから呼び出すとerrorとなるため、修正 2007-06-28
        if (value != undefined) {
          if (result[key]) {
...
Object.extend(Event, {
  ...
  //修正：unloadイベント前にunloadCacheするためにtop_elを追加
  _observeAndCache: function(element, name, observer, useCapture, top_el) {
  	//safariがdblclickのaddEventListenerに対応してないので修正
  	if (name == 'dblclick' &&
  ...
  observe: function(element, name, observer, useCapture, top_el) {
	...
	// Ryuji.M Opera10.10にてkeypressイベントがkeydownのイベントになり、Event.Stopしてもclickイベントが呼ばれてしまうため修正
    //if (name == 'keypress' &&
  ...
  _stopObserving: function(element, name, observer, useCapture) {
    ...
    // Ryuji.M Opera10.10にてkeypressイベントでのEvent.Stopができなくなり、clickイベントが呼ばれてしまうため修正
    //if (name == 'keypress' &&
    //    (navigator.appVersion.match(/Konqueror|Safari|KHTML/)
    //    || element.attachEvent))
    if (name == 'keypress' && !browser.isOpera &&
...
var Position = {
  ...
  //追加
  //cumulativeOffsetのdiv:scroll:autoも考慮したバージョン　by Ryuji.M
  //cumulativeOffsetは、offsetParentを用いており、cumulativeOffsetScroll(positionedOffsetScroll)は、parentNode
  //で検索するので、cumulativeOffsetのほうが高速。そのためcumulativeOffset(positionedOffset)はそのまま残す
  //
  cumulativeOffsetScroll: function(element) {
    ...
    do {
      ...
      element = parent_el;	//element.parentNode;
    } while (element);
    return [valueL, valueT];
  },
  //追加
  ...
  // 追加
  // 内幅(paddingを含み、スクロールバーを含む,border,marginを除く)を求める
...
/*
 * WYSIWYG内でdocument.writeするガジェットに対応するため(現状：FireFoxのみ動作)
 */

document._write = document.write;			    //別名保存
...
document.write = function(s) {
	...
	if(_nc_global_script_span.innerHTML != '') {
		if(current){
			...
			//new Insertion.After(document.body, _nc_global_script_span.innerHTML);
			if (document.body) {
...
function Browser() {
	...
	if ((i = ua.indexOf(s)) >= 0) {
		...
		if ((i = ua.indexOf(s)) >= 0) {
			this.isFirefox = true;
			// add firefoxの場合のみ処理を行っているが場合によっては、そのほかについても同様の処理にする必要あり
			var version = '';
...
function valueParseInt(param) {
	...
	//if (typeof param == 'string') param = param.replace(/^0*/, "");		//先頭の0の取り除き
