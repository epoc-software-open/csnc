...
class Crfif_Components_View
{
	...
	function get_belong_item( $user_id ) {
		...
		$sql = "SELECT i.item_id, l.content, hospital.hospital_code ".
				"FROM {items} i, {users_items_link} l ".
				" LEFT JOIN {hospitalinfo} hospital ON CONCAT( hospital.hospital_name, '|' ) = l.content ".
		...
		if ( $result ) {
			return rtrim( $result[0]['content'], '|' ) . "@" . $result[0]['hospital_code'];
		}
		return null;
	}
	...
	function edit_metadata( $record ) {
		...
		// 選択式(ラジオ, はい・いいえ, 該当せず・該当)
		if ( $record["type"] == 30 || $record["type"] == 34 || $record["type"] == 35 ) {
	...
	function edit_metadata_content( $record ) {
		...
		// 施設, 選択式(ラジオ, はい・いいえ, 該当せず・該当)
		if ( $record["type"] == 20 || $record["type"] == 30 || $record["type"] == 34 || $record["type"] == 35 ) {
...
/*
		...
		FROM netcommons2_users u
		  ...
		  LEFT JOIN netcommons2_hospitalinfo hospital ON hospital.hospital_name = CONCAT( belong.content, '|' )
		ORDER BY u.insert_time
*/

		$sql = "SELECT u.login_id, u.password, u.handle, u.role_authority_id, belong.content AS belong, email.content AS email, tedc.content AS tedc, hospital.hospital_code ".
				"FROM {users} u ".
				...
				" LEFT JOIN {hospitalinfo} hospital ON CONCAT( hospital.hospital_name, '|' ) = belong.content ".
				"ORDER BY u.insert_time";
...
foreach( $result as $record ) {
	...
	$tmp_belong = rtrim( $record['belong'], '|' ) . "@" . $record['hospital_code'];
...
EOF;
		...
		$sql = "SELECT t.content_no, t.insert_user_id, l.role_authority_id, t.insert_time " .
...
EOF;
		...
		$insert_time_jp = timezone_date( $insert_time, false, "Y/m/d H:i:s" );
...
print <<< EOF
    <metacontent>
      <metadata_content_id>4</metadata_content_id>
      <content>{$insert_time_jp}</content>
      <metadata_id>4</metadata_id>
      <metadata_name>登録日時</metadata_name>
    </metacontent>
