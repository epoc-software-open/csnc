...
class Uploads_View {
	...
	* @param int module_id
    ...
    function downloadCheck($upload_id, $show_auth_id = null, $thumbnail_flag = 0, $action_name = null, $force_resize=null) {
		...
		// ダウンロード履歴を保存する。 by nagahara@opensource-workshop.jp
		$this->insertDownloadlog( $uploads_obj[0] );
	 ...
	 * ダウンロード履歴を保存
	 */
	function insertDownloadlog( $upload_obj ) {
		...
		// ルーム名称取得
		$params = array ( 'page_id' => $upload_obj["room_id"] );
		$room_obj = $this->_db->selectExecute('pages', $params, null, 1);
		if ( $room_obj === false ) {
			return $room_obj;
		}

		$params = array();
		$params["upload_id"]          = $upload_obj["upload_id"];
		$params["room_id"]            = $upload_obj["room_id"];
		$params["module_id"]          = $upload_obj["module_id"];
		$params["unique_id"]          = $upload_obj["unique_id"];
		$params["file_name"]          = $upload_obj["file_name"];
		$params["physical_file_name"] = $upload_obj["physical_file_name"];
		$params["file_path"]          = $upload_obj["file_path"];
		$params["action_name"]        = $upload_obj["action_name"];
		$params["file_size"]          = $upload_obj["file_size"];
		$params["mimetype"]           = $upload_obj["mimetype"];
		$params["extension"]          = $upload_obj["extension"];
		$params["garbage_flag"]       = $upload_obj["garbage_flag"];
		$params["download_user_name"] = $user_name;
		$params["download_user_id"]   = $user_id;
		$params["download_ym_local"]  = date( "Ym" );
		$module_names = $this->loadModuleInfo( trim( $params["file_path"], "|" ) );
		$params["module_name"]        = $module_names["module_name"];
		$params["room_name"]          = $room_obj[0]["page_name"];

		$result = $this->_db->insertExecute( "downloadlog", $params, true, "downloadlog_id" );
		if( $result === false ) {
			return false;
		}
	}
