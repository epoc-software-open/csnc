...
/**
 * デバッグ・ログ設定 add by nagahara.
 */
require_once "nc_log.php";
 ...
 */
define('BASE_DIR', START_INDEX_DIR);
 ...
 */
define('INSTALLINC_PATH', START_INDEX_DIR . "/webapp/config/install.inc.php");
if(is_writeable(INSTALLINC_PATH) && (!isset($_POST['action']) || $_POST['action'] != "install_action_mdinstall_init")){
...
if(isset($_GET['action']) && $_GET['action'] == 'common_download_css') {
	require_once HTDOCS_DIR . "/htdocs/css.php";
} else if(isset($_GET['action']) && $_GET['action'] == 'common_download_js') {
	require_once HTDOCS_DIR . "/htdocs/js.php";
} else {
