 ...
 * @license     BSD, revised
 * @version     CVS: $Id: HTTP.php,v 1.1 2006/04/27 06:47:30 Ryuji.M Exp $
...
class HTTP
{
    ...
    function redirect($url, $exit = true, $rfc2616 = false)
    {
        ...
        if (    $rfc2616 && isset($_SERVER['REQUEST_METHOD']) &&
            ...
            printf('Redirecting to: <a href="%s">%s</a>.', htmlspecialchars($url, ENT_QUOTES), htmlspecialchars($url, ENT_QUOTES));
        }
        if ($exit) {
            exit;
        }
