 ...
 * @version    CVS: $Id: Mobile.php,v 1.2 2008/06/04 09:28:37 snakajima Exp $
...
class Net_UserAgent_Mobile
{
    ...
    function &factory($userAgent = null)
    {
        ...
        if (Net_UserAgent_Mobile::isDoCoMo($userAgent)) {
            ...
            $driver = 'Willcom';
        } elseif (Net_UserAgent_Mobile::isPortable($userAgent)) {
            $driver = 'Portable';
        } else {
            $driver = 'NonMobile';
        }
    ...
    /**
     * Checks whether or not the user agent is PSP or NintendoDS by a given user agent string.
     *
     * @param string $userAgent
     * @return boolean
     * @since Method available since Release 0.31.0
     */
    function isPortable($userAgent = null)
    {
        if (is_null($userAgent)) {
            $userAgent = @$_SERVER['HTTP_USER_AGENT'];
        }
