...
-- <b>Garbage Collection</b>: $qry</p>");
	...
	if ($rs && !$rs->EOF) {
		...
		if (abs($dbt - $t) >= ADODB_SESSION_SYNCH_SECS) {
			...
			$msg = 
			__FILE__.": Server time for webserver {".htmlspecialchars($_SERVER['HTTP_HOST'], ENT_QUOTES)."} not in synch with database: database=$dbt ($dbts), webserver=$t (diff=".(abs($dbt-$t)/3600)." hrs)";
			error_log($msg);
			if ($ADODB_SESS_DEBUG) ADOConnection::outp("
-- $msg</p>");
		}
