...
class DbObjectAdodb
{
	...
	function execute($sql, $params = array(), $limit = 0, $offset = 0, $key_flag = true, $func = null, $func_param = null) {
		...
		$target_sql_flag = false;
		$target_table_flag = false;

		// SQL 種別
		if ( ( stripos( trim( $sql ), "insert" ) === 0 ) || ( stripos( trim( $sql ), "update" ) === 0 ) || ( stripos( trim( $sql ), "delete" ) === 0 ) ) {
			$target_sql_flag = true;
		}

		// ターゲット・テーブル
		if ( stripos( trim( $sql ), "testee" ) !== false && stripos( trim( $sql ), "{testee_log}" ) === false ) {
			$target_table_flag = true;
		}

		if ( ( $target_sql_flag ) && ( $target_table_flag ) ) {

			$params_log = array( "exec_sql" => $sql,
			                     "param" => print_r($params, true),
			...
			// ターゲット・テーブル
			$sql_log .= $this->_prefix . "testee_log ";
			$sql_log .= "(exec_sql,param,insert_time,insert_user_id,insert_user_name) ";
			$sql_log .= "VALUES(?,?,?,?,?)";

			$result_log = $this->_conn->Execute($sql_log, $params_log);
		}
